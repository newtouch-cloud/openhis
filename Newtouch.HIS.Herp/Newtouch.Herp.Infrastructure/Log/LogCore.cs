...
namespace Newtouch.Herp.Infrastructure.Log
{
    ...
    {
        private static readonly ILog Logger = LogProxy.GetLogger(string.IsNullOrWhiteSpace(ConfigurationHelper.GetAppConfigValue("ServiceName")) ? "Newtouch.Herp" : ConfigurationHelper.GetAppConfigValue("ServiceName"));

        private static string LogAddress =
            string.IsNullOrEmpty(ConfigurationManager.AppSettings["settLogAddress"])
                ? "C:\\HISLog\\log_herp_web"

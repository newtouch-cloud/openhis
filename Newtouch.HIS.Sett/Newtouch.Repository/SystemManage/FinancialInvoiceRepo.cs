...
namespace Newtouch.HIS.Repository
{
    ...
    public class FinancialInvoiceRepo : RepositoryBase<FinancialInvoiceEntity>, IFinancialInvoiceRepo
    {
        ...
        {
            return this.IQueryable().Where(p => p.lyry == lyry && p.zt == "1" && p.is_del == 0 && p.OrganizeId == orgId)
        ...
        public void SubmitForm(FinancialInvoiceEntity financialInvoiceEntity, string fpdm)
        {
            ...
            {
                var sourceZt = financialInvoiceEntity.zt;
                financialInvoiceEntity.Create();
                financialInvoiceEntity.zt = sourceZt;
        ...
        {
            //作废或停用操作不做验证
            if (!string.IsNullOrWhiteSpace(Id) && (entity.is_del == 1 || entity.zt == "0"))
            {
                return;
            }

            if (entity.qsfph > entity.jsfph)
            {
                throw new FailedException("结束发票号必须大于起始发票号！");
            }
            if (entity.dqfph < entity.qsfph || entity.dqfph > entity.jsfph)
            {
                throw new FailedException("当前发票号必须介于起始发票号和结束发票号之间！");
            }
            var e = this.IQueryable().Where(p => p.is_del == 0 && p.OrganizeId == orgId && p.qsfph <= p.jsfph);
            ...
            if (e != null && e.Count() > 0)
            {
                ...
                {
                    if (string.IsNullOrWhiteSpace(entity.szm))
                    {
                        //首字母为空
                        if (string.IsNullOrWhiteSpace(item.szm))
                        {
                    ...
                    {
                        //首字母不为空
                        if (!string.IsNullOrWhiteSpace(item.szm) && entity.szm.Trim() == item.szm.Trim())
                        {
                            if ((entity.qsfph < item.qsfph && entity.jsfph > item.jsfph) ||
                                (entity.qsfph < item.qsfph && entity.jsfph > item.qsfph) ||
                                (entity.qsfph > item.qsfph && entity.jsfph < item.jsfph) ||
                ...
                if (entity.zt == "1" && e.Where(p => p.zt == "1" && p.lyry == entity.lyry).Count() > 0)
                {
                    throw new FailedException("已存在生效的发票号段！");
                }
            }
        }
    }

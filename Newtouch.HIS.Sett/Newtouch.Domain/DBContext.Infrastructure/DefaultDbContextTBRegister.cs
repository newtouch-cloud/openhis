...
namespace Newtouch.HIS.Domain.DBContext.Infrastructure
{
    ...
    public class DefaultDbContextTBRegister
    {
        ...
        public static void Registe(DbModelBuilder modelBuilder)
        {
            ...
            //订单
            modelBuilder.Entity<OrderInfoEntity>().RegisterTable().HasKey(p => p.Id);
            modelBuilder.Entity<OrderMzEntity>().RegisterTable().HasKey(p => p.Id);
            modelBuilder.Entity<OrderZyEntity>().RegisterTable().HasKey(p => p.Id);
        }

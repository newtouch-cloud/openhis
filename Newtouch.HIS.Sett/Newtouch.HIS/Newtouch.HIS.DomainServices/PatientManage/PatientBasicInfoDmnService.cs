...
FROM     xt_brjbxx  jb
        ...
        public string SaveSysBasicAccountInfo(SysHosBasicInfoVO VO, string OrganizeId, out string res)
        {
            ...
            hospPatientBasicInfo.CardType = ((int)EnumCardType.XNK).ToString();
            hospPatientBasicInfo.CardTypeName = EnumCardType.XNK.GetDescription();
            if (!string.IsNullOrEmpty(VO.brxz) && VO.brxz == "1")
            {
                hospPatientBasicInfo.CardType = VO.cardtype;
                hospPatientBasicInfo.CardTypeName = VO.CardTypeName;
            }
            if (!string.IsNullOrEmpty(VO.brxz) && VO.brxz == "8")
            {
                hospPatientBasicInfo.CardType = ((int)EnumCardType.XNHJYK).ToString();
                hospPatientBasicInfo.CardTypeName = EnumCardType.XNHJYK.GetDescription();
            }
...
where sl <> 0";
        ...
        public List<YibaoDataVo> GetYbCancelList(Pagination pagination, string isjs, string lx, string keyword)
        {
            ...
            if (isjs == "yj")//医保已结算 HIS未结算
            {
                if (lx == "zy")
                {
                    strsql.Append(@"select mdtrt_id,setl_id,psn_no,psn_name,setl_time,a.zyh zymzh,medfee_sumamt,medins_setl_id,kh.cbdbm from drjk_zyjs_output a
left join cqyb_OutPut05 b on b.jylsh=a.setl_id and b.zt='1'
left join zy_brjbxx c on c.zyh=a.zyh
left join xt_card kh on kh.cardno=c.kh and c.OrganizeId=kh.OrganizeId
where a.zt='1' and b.jylsh is null ");

                    if (!string.IsNullOrEmpty(keyword))
                    {
                        strsql.Append(" and (psn_name like @keyword or a.zyh like @keyword)");
                        sqlparam.Add(new SqlParameter("@keyword", "%" + keyword.Trim() + "%"));
                    }
                }
                else
                {
                    strsql.Append(@"select mdtrt_id,setl_id,psn_no,psn_name,setl_time,a.mzh zymzh,medfee_sumamt,medins_setl_id,kh.cbdbm from drjk_mzjs_output a
left join cqyb_OutPut05 b on b.jylsh=a.setl_id and b.zt='1'
left join mz_gh c on c.mzh=a.mzh
left join xt_card kh on kh.cardno=c.kh and c.OrganizeId=kh.OrganizeId
where a.zt='1'  and a.setl_time>CONVERT(varchar(8),GETDATE(),121)+'01' and b.jylsh is null");

                    if (!string.IsNullOrEmpty(keyword))
                    {
                        strsql.Append(" and (psn_name like @keyword or a.mzh like @keyword)");
                        sqlparam.Add(new SqlParameter("@keyword", "%" + keyword.Trim() + "%"));
                    }
                }
            }
            ...
            {
                strsql.Append(@"select d.mdtrt_id mdtrt_id,'' setl_id,kh.grbh psn_no,b.xm psn_name,a.zyh zymzh,
SUBSTRING(l.inHead,CHARINDEX(',',l.inHead)+6,CHARINDEX(',',l.inHead,CHARINDEX(',',l.inHead)+1)-CHARINDEX(',',l.inHead)-6 ) medins_setl_id,
kh.cbdbm,l.errorMsg errormsg,l.tradiNumber infno from zy_brjbxx a
left join xt_brjbxx b on a.patid=b.patid and a.OrganizeId=b.OrganizeId and b.zt='1'
left join xt_card kh on kh.CardNo=a.kh  and a.kh=kh.CardNo 
left join cqyb_OutPut02 c on a.zyh=c.zymzh and a.OrganizeId=c.OrganizeId and c.zt='1'
left join drjk_rybl_input d on d.zyh=a.zyh and d.zt='1'
left join ybjk_logcontent l on l.hisId=a.zyh  and l.errorMsg='' and l.tradiNumber='2401'
where  zybz not in('3','9') and a.zt='1' ");
            ...
            {
                strsql.Append(@" select ry.mdtrt_id,'' setl_id,kh.grbh psn_no,b.xm psn_name, a.zyh zymzh,
SUBSTRING(l.inHead,CHARINDEX(',',l.inHead)+6,CHARINDEX(',',l.inHead,CHARINDEX(',',l.inHead)+1)-CHARINDEX(',',l.inHead)-6 ) medins_setl_id,
 kh.cbdbm,l.errormsg errormsg,l.tradiNumber infno from zy_brjbxx a
left join xt_brjbxx b on a.patid=b.patid and a.OrganizeId=b.OrganizeId and b.zt='1'
left join xt_card kh on kh.CardNo=a.kh and kh.OrganizeId=a.OrganizeId
left join drjk_rybl_input ry on ry.zyh=a.zyh  --and ry.psn_no is null
inner join ybjk_logcontent l on l.hisId=a.zyh  and l.errorMsg<>'' and l.tradiNumber='2304'
where kh.cblb<>'3' and a.brxz='1' and zybz not in('3','9') and a.zt='1'");
                if (!string.IsNullOrEmpty(keyword))
                {
                    strsql.Append(" and (b.xm like @keyword or a.zyh like @keyword)");
                    //sqlparam.Add(new SqlParameter("@keyword", "%" + keyword.Trim() + "%"));
                }
                strsql.Append(@" union all select  a.jzid mdtrt_id,'' setl_id,kh.grbh psn_no,b.xm psn_name, a.mzh zymzh,
SUBSTRING(l.inHead,CHARINDEX(',',l.inHead)+6,CHARINDEX(',',l.inHead,CHARINDEX(',',l.inHead)+1)-CHARINDEX(',',l.inHead)-6 ) medins_setl_id,
 kh.cbdbm,l.errormsg errormsg,l.tradiNumber infno from mz_gh a
left join xt_brjbxx b on a.patid=b.patid and a.OrganizeId=b.OrganizeId and b.zt='1'
left join xt_card kh on kh.CardNo=a.kh and kh.OrganizeId=a.OrganizeId
inner join ybjk_logcontent l on l.hisId=a.mzh  and l.errorMsg='0' and l.tradiNumber='2207'
where kh.cblb<>'3' and a.brxz='1'  and a.zt='1'  ");
                if (!string.IsNullOrEmpty(keyword))
                {
...
WHERE   a.zyh = @zyh
        ...
        public void OutPatZFchangetoYB(string orgId, string mzh, int patid, ZYToYBDto patInfo)
        {
            ...
            {
                throw new FailedException("参数异常，证件号缺失");
            }
            if (string.IsNullOrWhiteSpace(patInfo.kh))
            {
                throw new FailedException("参数异常，卡号缺失");
            }
            using (var db = new EFDbTransaction(this._databaseFactory).BeginTrans())
            {
                var oldpatid = 0;
                var ghxxEntity = db.IQueryable<OutpatientRegistEntity>(p => p.mzh == mzh && p.OrganizeId == orgId).First();
                //if (ghxxEntity.patid != patid)
                //{
                // throw new FailedException("异常：patid错误");
                //}
                oldpatid = ghxxEntity.patid;
                //更新zy_brjbxx
                ghxxEntity.brxz = "1";    //普通医保
                ghxxEntity.zjlx = ((int)EnumZJLX.sfz).ToString();
                ghxxEntity.zjh = patInfo.sfzh;
                ghxxEntity.kh = patInfo.kh;
                ghxxEntity.CardType = ((int)EnumCardType.YBJYK).ToString();
                ghxxEntity.CardTypeName = EnumCardType.YBJYK.GetDescription();
               ...
               // }
                ...
                //xtbrxxEntity.cbdbm = patInfo.cbdbm;
                //xtbrxxEntity.xzlx = patInfo.xzlx;
                //xtbrxxEntity.grbh = patInfo.grbh;
                //...
                xtbrxxEntity.Modify();
                db.Update(xtbrxxEntity);
        ...
        public void InPatZFchangetoYB(string orgId, string zyh, int patid, ZYToYBDto patInfo, CqybMedicalReg02Entity ryblInfo)
        {
            ...
            {
                throw new FailedException("医保地未设置");
            }
        ...
        public SysPatBasicInfoVo ValidateFirstVisit(string sfzh, string xm,string orgId,string kh=null,string jzpzlx=null)
        {
            ...
            strSql.AppendFormat(@"select  xtxx.patid,xtxx.zjh,
                                  ...
                                  where ((CardNo=@kh and CardType=@jzpzlx)
                                    ...
                                    and xm=@xm and xtxx.OrganizeId=@orgId
                                    and xtxx.zt='1' and kh.brxz<>'0'");
...
where a.zt='1' and a.jsnm=@jsnm";
  ...
  public bool updateZybrxxkExpandZhye(string zyh, string organizeId,decimal? zhye) {
        ...
        {
            string sqlstr = @"select code,name,py,convert(varchar(10),mzzybz)mzzybz,zxks,ybksbm,bmdm from NewtouchHIS_Base..[Sys_Department] where zt='1' and OrganizeId=@orgid";

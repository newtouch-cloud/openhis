...
namespace Newtouch.HIS.DomainServices
{
    public class OutPatientChargeQueryDmnService : DmnServiceBase, IOutPatientChargeQueryDmnService
    {
        ...
        /// <param name="createTimeEnd"></param>
        /// <returns></returns>
        public IList<OutPatientRegChargeMVO> RegChargeQuery(Pagination pagination, string kh, string fph,string xm, string syy, DateTime? createTimestart, DateTime? createTimeEnd, DateTime? sfrqTimestart, DateTime? sfrqTimeEnd)
        {
...
SELECT js.blh, js.jsnm,js.jslx,
       ...
       ISNULL(js.LastModifierCode, '') tCreatorCode,
		...
		brxz.brxzmc,
        gh.zdmc,jzys.Name jzys,xtjz.zlkssj jzsj
...
");
            ...
            {
                sqlStr.AppendLine("AND js.fph LIKE @fph");
...
and mzjszffs.OrganizeId = @orgId";
        ...
        public IList<OutPatientRegChargeDetailsVO> GetRecordsByJsnm(string jsnm)
        {
            ...
            var sql = @"select '' cfh,sfxm,ypmc,dw,dlmc, 
	sum(dj) dj, sum(sl) sl, sum(je) AS je,sum(klsl) klsl,sum(klje) klje,sum(ytsl) ytsl,sum(ytje) ytje,ysmc from (
	SELECT '' cfh, (case when xm.ztId is not null then xm.ztId else xm.sfxm end) sfxm, (case when xm.ztmc is not null then xm.ztmc else sfxm.sfxmmc end) AS ypmc,
	...
	sum(xm.dj) dj, sum(jsmx.sl) sl, sum(jsmx.sl*xm.dj) AS je,sum(xm.sl) klsl,sum(xm.je) klje,sum(xm.sl-jsmx.sl) ytsl,sum(xm.je-(jsmx.sl*xm.dj)) ytje,xm.ysmc,xm.ztId,xm.ztmc 
	--into #js_pay
	...
	group by xm.ztId,xm.ztmc,xm.sfxm,sfxm.sfxmmc,xm.dw,sfdl.dlmc,xm.ysmc
	) a group by ztId,ztmc,sfxm,ypmc,dw,dlmc,ysmc,cfh
	UNION ALL
	SELECT cf.cfh,cfmx.yp sfxm, yp.ypmc, cfmx.dw dw, sfdl.dlmc, 
	cfmx.dj, jsmx.sl, (jsmx.sl*dj) AS je,cfmx.sl klsl,cfmx.je klje,cfmx.sl-jsmx.sl ytsl,cfmx.je-(jsmx.sl*cfmx.dj) ytje,cf.ysmc
	FROM dbo.mz_jsmx jsmx

...
namespace Newtouch.HIS.Web.Areas.OutpatientManage.Controllers
{
    ...
    public class OutpatientChargeQueryController : ControllerBase
    {
        ...
        /// <returns></returns>
        [HandlerAjaxOnly]
        public ActionResult SelectRegChargeQuery(Pagination pagination, DateTime? createTimestart, DateTime? createTimeEnd, DateTime? sfrqTimestart, DateTime? sfrqTimeEnd, string kh = "", string fph = "", string xm = "", string syy = "")
        {
            IList<OutPatientRegChargeMVO> list;
            var chargeQueryList = new
            {
                rows = (list = _outPatienChargeQueryDmnService.RegChargeQuery(pagination, kh, fph, xm, syy, createTimestart, createTimeEnd, sfrqTimestart, sfrqTimeEnd)),

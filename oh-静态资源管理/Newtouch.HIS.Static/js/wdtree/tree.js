(function ($) {
    ...
    $.fn.treeview = function (settings) {
        var dfop =
            {
                ...
                cbiconpath: "http://59.80.30.157:20103/js/wdtree/images/icons/",
            ...
            if (data && data.length > 0) {
                ...
                asnyloadc(null, false, function (data) {
                    if (data && data.length > 0) {
                        treenodes = data;
                        dfop.data = data;
                        if (typeof dfop.dataCallback === 'function') {
                            dfop.dataCallback(data);
                        }
                        if (dfop.description) {
                            data.unshift({
                                "id": "",
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            ht.push("<span data-id='" + nd.id + "' data-value='" + nd.value + "'");
            if (nd.Code) {
                ht.push(" data-Code='" + nd.Code + "'");
            }
            if (nd.Ex1) {
                ht.push(" data-Ex1='" + nd.Ex1 + "'");
            }
            if (nd.Ex2) {
                ht.push(" data-Ex2='" + nd.Ex2 + "'");
            }
            if (nd.Ex3) {
                ht.push(" data-Ex3='" + nd.Ex3 + "'");
            }
            if (nd.Ex4) {
                ht.push(" data-Ex4='" + nd.Ex4 + "'");
            }
            if (nd.Ex5) {
                ht.push(" data-Ex5='" + nd.Ex5 + "'");
            }
            if (nd.Ex6) {
                ht.push(" data-Ex6='" + nd.Ex6 + "'");
            }
            ht.push(" class='bbit-tree-node-text' unselectable='on'>", nd.text, "</span>");
            ht.push("</a>");
        ...
        function nodeclick(e) {
            ...
            if (et.tagName == "IMG") {
                ...
                //触发checkbox click
                if (dfop.nodeTextTriggerCb == true && dfop.showcheck === true && $(this).attr('id')) {
                    var $cbs = $('img#' + $(this).attr('id') + '_cb');
                    if ($cbs.length == 1) { 
                        $cbs.trigger('click');
                        return false;
                    }
        ...
        function nodedbclick(e) {
            ...
            if (et.tagName == "IMG") {
                ...
                $("." + id).find('div').removeClass("bbit-tree-selected");
                $(this).addClass("bbit-tree-selected");
                if (dfop.onnodedbclick) {
                    if (!item.expand) {
                        item.expand = function () { expandnode.call(item); };
                    }
                    dfop.onnodedbclick.call(this, item);
                }
            }
        }
        function expandnode() {
    ...
    $.fn.getCheckedValues = function () {
        ...
        $id.find('.bbit-tree-node-cb').each(function () {
            ...
            _src = _src.substr(_src.lastIndexOf("/") + 1);
            if (_src == 'checkbox_1.png' || _src == 'checkbox_2.png') {
                var $dObj = $(this).siblings('a.bbit-tree-node-anchor').find('.bbit-tree-node-text');
                var _value = $dObj.attr('data-value');
                value.push(_value);
            }
        });
        return value;
    };
    $.fn.getCheckedCodes = function () {
        var $id = $(this);
        //var _length = $id.attr('id').trim().length + 1;
        var value = []
        $id.find('.bbit-tree-node-cb').each(function () {
            ...
            if (_src == 'checkbox_1.png' || _src == 'checkbox_2.png') {
                //处于选择状态
                var $dObj = $(this).siblings('a.bbit-tree-node-anchor').find('.bbit-tree-node-text');
                var _value = $dObj.attr('data-Code');
                value.push(_value);
            }
        });
        return value;
    ...
    //获取对象数组
    $.fn.getCheckedNodeObjArray = function () {
        var value = []
        $(this).find('.bbit-tree-node-cb').each(function () {
            var _src = $(this).attr('src');
            _src = _src.substr(_src.lastIndexOf("/") + 1);
            if (_src == 'checkbox_1.png' || _src == 'checkbox_2.png') {
                //处于选择状态
                var $dObj = $(this).siblings('a.bbit-tree-node-anchor').find('.bbit-tree-node-text');
                var temmmmmmmmmp = {
                    id: $dObj.attr('data-id'), value: $dObj.attr('data-value'), text: $.trim($dObj.html())
                };
                if ($dObj.attr('data-Code')) {
                    temmmmmmmmmp.Code = $dObj.attr('data-Code');
                }
                if ($dObj.attr('data-Ex1')) {
                    temmmmmmmmmp.Ex1 = $dObj.attr('data-Ex1');
                }
                if ($dObj.attr('data-Ex2')) {
                    temmmmmmmmmp.Ex2 = $dObj.attr('data-Ex2');
                }
                if ($dObj.attr('data-Ex3')) {
                    temmmmmmmmmp.Ex3 = $dObj.attr('data-Ex3');
                }
                if ($dObj.attr('data-Ex4')) {
                    temmmmmmmmmp.Ex4 = $dObj.attr('data-Ex4');
                }
                if ($dObj.attr('data-Ex5')) {
                    temmmmmmmmmp.Ex5 = $dObj.attr('data-Ex5');
                }
                if ($dObj.attr('data-Ex6')) {
                    temmmmmmmmmp.Ex6 = $dObj.attr('data-Ex6');
                }
                value.push(temmmmmmmmmp);
            }
        });
        return value;
    };

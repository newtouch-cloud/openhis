    ...
    c.extend(c.validator, {
        ...
        prototype: {
            ...
            showErrors: function (a) {
                ...
                if (this.errorList && this.errorList.length && this.errorList.length > 0) {
        ...
        methods: {
            ...
            mobilephone: function (a, b) {
                return this.optional(b) || /^(13|15|17|18|19)\d{9}$/.test(a)
            }
        }
    });
...
$(function () {
    ...
    jQuery.validator.addMethod("isMobile", function (value, element) {
        ...
        return this.optional(element) || (length == 11 && /^[1][0-9][0-9]{9}$/.test(value));
    }, "请正确填写您的手机号码。");
    // 电话号码验证    
    ...
    jQuery.validator.addMethod("isIdentity", function (value, element) {
        var iSum = 0;
        var result = true;
        if (!/^\d{17}(\d|x)$/i.test(value)) result = false; return result;
        value = value.replace(/x$/i, "a");
        if (aCity[parseInt(value.substr(0, 2))] == null) result = false; return result;
        sBirthday = value.substr(6, 4) + "-" + Number(value.substr(10, 2)) + "-" + Number(value.substr(12, 2));
        var d = new Date(sBirthday.replace(/-/g, "/"));
        if (sBirthday != (d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate())) result = false; return result;
        for (var i = 17; i >= 0; i--) iSum += (Math.pow(2, i) % 11) * parseInt(value.charAt(17 - i), 11);
        if (iSum % 11 != 1) result = false; return result;

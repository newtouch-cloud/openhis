...
namespace FrameworkBase.Oracle.Repository
{
    ...
    public sealed class SysConfigRepo : RepositoryBase<SysConfigEntity>, ISysConfigRepo
    {
        ...
        {
            var sql = "select * from \"Sys_Config\" where (\"Code\" like :searchKeyword or \"Name\" like :searchKeyword) order by \"Code\" asc";
            return this.FindList<SysConfigEntity>(sql, new OracleParameter[] {
                    new OracleParameter(":searchKeyword", "%" + (keyword ?? "") + "%")
                });
        }

...
{
    public class HospitalizatiionReturnMedicineProcessV2 : ProcessorFun<tyInfo>
    {
        private readonly List<HospitalizationReturnDrugParem> _effectiveLs;

        public HospitalizatiionReturnMedicineProcessV2(tyInfo request) : base(request)
        {
        ...
        private void SinglethreadingAction(ActResult actResult)
        {
            ...
            {
                var tmpResult = new DispenseIndexInfoDmnService(new DefaultDatabaseFactory()).HospitalizatiionReturnMedicineV2(p, Request.zytyapplyno);
                ...
                var parent = new Task(() =>
                {
                    ...
                    foreach (var item in _effectiveLs)
                    {
                        childFactory.StartNew(() =>
                        {
                            ...
                            var tmpResult = new DispenseIndexInfoDmnService(new DefaultDatabaseFactory()).HospitalizatiionReturnMedicineV2(item, Request.zytyapplyno);

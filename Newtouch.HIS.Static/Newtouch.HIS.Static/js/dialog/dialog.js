...
! function(a, b) {
    ...
    var c, d, e = {
        ...
        f = {
            ...
            config: function(a, b) {
                var d = 0;
                return a = a || {}, f.cache = e.config = c.extend(e.config, a), f.path = e.config.path || f.path, "string" == typeof a.extend && (a.extend = [a.extend]), f.use("dialog.css", a.extend && a.extend.length > 0 ? function g() {
                    var c = a.extend;
    ...
    h.anim = ["layer-anim", "layer-anim-01", "layer-anim-02", "layer-anim-03", "layer-anim-04", "layer-anim-05", "layer-anim-06"], g.pt.config = {
        ...
        return f.zIndex = g, b([f.shade ? '<div class="layui-layer-shade" id="layui-layer-shade' + d + '" times="' + d + '" style="' + ("z-index:" + (g - 1) + "; background-color:" + (f.shade[1] || "#000") + "; opacity:" + (f.shade[0] || f.shade) + "; filter:alpha(opacity=" + (100 * f.shade[0] || 100 * f.shade) + ");") + '"></div>' : "", '<div class="' + h[0] + " " + (h.anim[f.shift] || "") + (" layui-layer-" + e.type[f.type]) + (0 != f.type && 2 != f.type || f.shade ? "" : " layui-layer-border") + " " + (f.skin || "") + '" id="' + h[0] + d + '" type="' + e.type[f.type] + '" times="' + d + '" showtime="' + f.time + '" conType="' + (a ? "object" : "string") + '" style="z-index: ' + g + "; width:" + f.area[0] + ";height:" + f.area[1] + (f.fix ? "" : ";position:absolute;") + '">' + (a && 2 != f.type ? "" : k) + '<div id="' + (f.id || "") + '" class="layui-layer-content' + (0 == f.type && -1 !== f.icon ? " layui-layer-padding" : "") + (3 == f.type ? " layui-layer-loading" + f.icon : "") + '">' + (0 == f.type && -1 !== f.icon ? '<i class="fa ' + f.icon + '"></i>' : "") + (1 == f.type && a ? "" : f.content || "") + '</div><span class="layui-layer-setwin">' + function() {
            var a = j ? '<a class="layui-layer-min" href="javascript:;"><cite></cite></a><a class="layui-layer-ico layui-layer-max" href="javascript:;"></a>' : "";
            return f.closeBtn && (a += '<a class="layui-layer-close layui-layer-close1" href="javascript:;">×</a>'), a
        }() + "</span>" + (f.btn ? function() {
            var a = "";
            "string" == typeof f.btn && (f.btn = [f.btn]);
            for (var b = 0, c = f.btn.length; c > b; b++) a += '<a class="' + h[6] + b + ' ' + f.btnclass[b] + '">' + f.btn[b] + "</a>";
           var lefthtml = "";
            if (f.type == 2&&f.showleftlalbel) {
                var leftlabel = f.leftlabel;
                lefthtml = '<a class="layui-layer-checkbox"><div class="ckbox"><input id="leftlabel" class="footer-left-label" type="checkbox" ' + (!!leftlabel.checked ? 'checked' : '') +
                 '><label for="leftlabel" style="color: #0bb8d0;margin-left: 5px;font-weight: bolder;">' + leftlabel.text + '</label> </div></a>';
            }
            return '<div class="' + h[6] + '">'  + lefthtml + a + "</div>"
        }() : "") + "</div>"], k), c
    }, g.pt.creat = function() {
        var a = this,
            b = a.config,
        ...
        b.openLayer(), g.success && (2 == g.type ? d.find("iframe").on("load", function() {
            ...
            var a = c(this).index();
            if (0 === a) {
                 if ($(this).find('input').attr('class') != "footer-left-label")

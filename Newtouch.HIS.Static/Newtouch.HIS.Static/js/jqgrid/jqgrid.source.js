...
(function (factory) {
        ...
        styleUI: {
            ...
            Bootstrap: {
                ...
                base: {
                    ...
                    //icon_first: "glyphicon-step-forward",
                    //icon_prev: "glyphicon-forward",
                    //icon_next: "glyphicon-backward",
                    //icon_end: "glyphicon-step-backward",
    ...
    $.fn.jqGrid = function (pin) {
        ...
        return this.each(function () {
            ...
            var p = $.extend(true, {
                ...
                scrollrows: false,
                autowidth: false,
                scrollOffset: 23,//scrollOffset: 18,
                cellLayout: 5,
                subGridWidth: 20,
                multiselectWidth: 30,
                gridview: true,
            ...
            //
            if (p.treeMultiSelect === true) {
                for (i = 0; i < p.colModel.length; i++) {
                    if (p.colModel[i].showckbox === true) {
                        p.colModel[i].formatter = function (cellvalue, options, cell) {
                            var thisrowId = options.rowId;
                            if (!!cellvalue && !!thisrowId) {
                                var checkboxhtml = '<label>' +
                                                    '<input type="checkbox" id="chx' + thisrowId + '" class="ace" value="' + thisrowId + '" onclick="' + p.id + '_clickCheckbox(\'' + thisrowId + '\');" />' +
                                                    '<span class="lbl align-top" ></span>' +
                                                        cellvalue +
                                                '</label>';
                                //var checkboxhtml = "<div class=\"ckbox\"><input role=\"checkbox\" type=\"checkbox\"" + " id=\"cb_" + thisrowId + "\"><label for=\"cb_" + thisrowId + "\"></label></div>";
                                return checkboxhtml;
                            } else {
                                return "";
                            }
            ...
            addMulti = function (rowid, pos, irow, checked, uiclass) {
                ...
                //return "<td role=\"gridcell\" " + prp + ">" + v + "</td>";
                var v = "<div class=\"ckbox\"><input role=\"checkbox\" type=\"checkbox\"" + " id=\"jqg_" + ts.p.id + "_" + rowid + "\"><label for=\"jqg_" + ts.p.id + "_" + rowid + "\"></label></div>";
                var prp = formatCol(pos, irow, '', null, rowid, true);
                return "<td role=\"gridcell\" " + prp + ">" + v + "</td>";
            },
            ...
            addJSONData = function (data, rcnt, more, adjust) {
                ...
                if (!more) { ts.updatepager(false, true, data); }
                if (locdata) {
            ...
            updatepager = function (rn, dnd, data) {
                ...
                //将服务器ajax返回的完整data 回调给gridComplete
                if ($.isFunction(ts.p.gridComplete)) { ts.p.gridComplete.call(ts, data); }
                $(ts).triggerHandler("jqGridAfterGridComplete");
            },
            beginReq = function () {
                if (!(ts.p.showloadingLayer === false)) {
            ...
            endReq = function () {
                //IEbug 得变一次宽度
                if ($.isFunction(window.reSetGridWidth) && $.isFunction(window.isIE) && window.isIE()) {
                    window.reSetGridWidth();
                }
                if (!(ts.p.showloadingLayer === false)) {
                    window.setTimeout(function () {("#loadingPage").find('.loading-content').removeAttr('isTableLoading');
                        $.loading(false);
                    }, 0);
                }
                ...
                if (ts.p.unwritten) {
                    ...
                    if ($("#" + ts.p.id).find('tbody tr').length == 1) {
                        $("#" + ts.p.id).parents('.ui-jqgrid-bdiv').append('<div class="unwritten" style="color:red;padding:7px;border-bottom:#ddd 1px solid;">' + ts.p.unwrittentext + '</div>')
                    }
                }
            },
            ...
            setPager = function (pgid, tp) {
                ...
                if (twd > 0) {
                    ...
                    //$("td#" + pgid + "_" + ts.p.pagerpos, "#" + pgcnt).width(twd);   //by caishanshan
                }
                ts.p._nvtd = [];
            ...
            $(ts).before(grid.hDiv).click(function (e) {
                ...
                if (e.target.tagName === 'LABEL') {
                    $(e.target).closest('td').trigger('click'); //点chekbox有bug，改为触发td
                    return false;
                }
    ...
    $.jgrid.extend({
        ...
        getRowData_AllLine: function (justSelarrrow, returnObject) {
            try{
                var resall = [];
                var $t = this;
                if (justSelarrrow// && $t.p.multiselect
                    ) {
                    $.each($($t).jqGrid('getGridParam', 'selarrrow'), function () {
                        var thisRowData = $($t).jqGrid('getRowData', this);
                        thisRowData.jqRowId = this; //rowid
                        resall.push(thisRowData);
                    });
                }
                else {
                    $.each($($t).getDataIDs(), function () {    //默认所有行
                        var thisRowData = $($t).jqGrid('getRowData', this);
                        thisRowData.jqRowId = this; //rowid
                        resall.push(thisRowData);
                    });
                ...
                if (!(returnObject === false)) {
                    ...
                    return JSON.stringify(resall);  //默认return json
                }
            }
            catch (err) {
                if (!(returnObject === false)) {
        ...
        setGridWidth: function (nwidth, shrink) {
            return this.each(function () {
                ...
                if (shrink === true) {
                    ...
                    cr = 0;
                    if (hs) {
                        if (nwidth - gw - (initwidth + brd * vc) !== scw * 2) { //if (nwidth - gw - (initwidth + brd * vc) !== scw) {
                            cr = nwidth - gw - (initwidth + brd * vc) - scw * 2;    //cr = nwidth - gw - (initwidth + brd * vc) - scw;
                        }
                    ...
                    if (!hs && this.p.rownumbers) {
                        $t.p.colModel[lvc].width -= (this.p.rownumWidth + 4); //用最后一列除去row的width    //row的border
                        $t.p.tblwidth -= 2;
                    }
                ...
                if ($t.p.tblwidth) {
                    ...
                    $('table:first', $t.grid.hDiv).css("width", ($t.p.tblwidth + 2) + "px");
                    ...
                    $('td[role="gridcell"]', ind).each(function (i) {
                        ...
                        if (nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
                            ...
                            if (cm[i].editable === true) {
                                ...
                                $(elc).css('height', '99%');
                                if (cm[i].editwidth) {
                                    $(elc).css('width', cm[i].editwidth);
                                }
                                else {
                                    $(elc).css('width', '40%');
                                }
                                if ($t.p.editinputborderradius === false) {
                                    $(elc).css('border-radius', '0');
                                }
                                if ($t.p.editinputwidthborder === false) {
                                    $(elc).css('border', '0');
                                }
                                cnt++;
                            }
    ...
    */
    ...
    $.fn.fmatter.checkbox = function (cval, opts) {
        ...
        return "<div class=\"ckbox\"><input role=\"checkbox\" type=\"checkbox\" " + bchk + " id=\"cb_" + this.p.id + "_" + opts.rowId + "\"><label for=\"cb_" + this.p.id + "_" + opts.rowId + "\"></label></div>";
    };
